#!/usr/bin/env python3
"""
ACSL 备赛 PPT 生成器（v1.0）

用法：
    python _new_lesson.py N "标题" /tmp/lN_body.html TOTAL

参数：
    1. 课次（如 6 表示 L6）
    2. 标题（如 "递归函数"）
    3. body 文件路径（含 <section>...</section>...）
    4. 总页数（如 20）
...
def create_lesson(lesson_num, title, body_file, total):
    ...
    new_content = re.sub(
        r'<title>.*?</title>',
        f'<title>L{lesson_num} · {title} — ACSL 备赛</title>',
        new_content,
        count=1
    )
...
def main():
    if len(sys.argv) != 5:
        ...
        print('示例：python _new_lesson.py 6 "递归函数" /tmp/l6_body.html 20')
        sys.exit(1)
